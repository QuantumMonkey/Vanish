...
The Machine Hygiene panel puts a DECISION BAR above that verdict. The verdict
says what state the machine is in; the bar says what to do about it, with one
recommended click. Its model is a pure function -- hygieneDecisionActions in
renderer/hygiene.js, no DOM and no globals -- so the rules it encodes are
asserted directly rather than inferred from rendered markup.

Three of those rules are load-bearing. The recommendation is chosen by what a
finding would COST to get back and never by its size, matching how
lib/findings.js ranks the list and for the same reason. No action at all is
offered while a scan is running, because a recommendation from a partial run
is not one. And no action is computed from a byte total that decide() reports
as untrustworthy -- the control is disabled with its reason rendered on the
page, since a control the operator can click that the system will refuse is a
promise the screen cannot keep.

Nothing the bar offers removes anything. The panel is audit-only in both
modes, and the bar carries that sentence in every state it can render.
