...
Register-SharedWalkHarvest -markerNames @('pubspec.yaml')
...
function script:Find-PiuFlutterProjectDirs {
    <#
    .SYNOPSIS
        Directories under $root that contain a pubspec.yaml, via the one walk this finder now
    ...
    $walk = Invoke-SharedTreeWalk -root $root -maxDepth $maxDepth -maxDirs $maxDirs -skipDirs $script:PiuFlutterSkipDirs
    return @{
        projects    = @($walk.markers['pubspec.yaml'])
