...
- **Machine Hygiene still takes a minute or two.** The thirteen checks measured **81 seconds** on their own, and **94-101 seconds** driving the real panel with the rest of the app loading beside it, on the machine it was built on — they walk your profile and hash file contents, so this scales with what is on your disk. It was over ten minutes before 0.9.1 with one check that never finished, and 107 seconds before 0.9.2 made four checks share one walk instead of taking four. The floor is a single check, duplicate content, at 54-60 seconds: it hashes file contents, no amount of parallelism goes below one check, and nothing about the other twelve can move it.
