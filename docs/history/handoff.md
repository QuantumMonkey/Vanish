...
* Work was committed on branch `feat/core-tier-stages-3-6-9`, off `master`, per the `01-trd.md` git convention, at the time of this snapshot. That branch has since merged; as of 2026-08-03 all work (including the `/cso` fixes and the startup elevation toggle) is committed directly to `master`.
