...
function writeSettings(patch) {
  ...
  // wy7a: the floor is 1, not 0, and THIS is the line that enforces it - the
  // min="1" on the two number inputs is a convenience, and INV-2 says so
  // explicitly ("the renderer's disabled states are a convenience; THIS is the
  // boundary"). clampInt clamps rather than rejecting, so a 0 arriving from
  // anywhere - a hand-edited settings.json, an older file, a renderer that
  // does not exist yet - becomes 1 rather than being honoured.
  //
  // Retention 0 makes autoPurgeSweep's cutoff `now`, and it deletes forever
  // every entry older than the cutoff at the next elevated start. That is not
  // a retention policy, it is "empty the vault at next launch" spelled as a
  // number, one keystroke below the default, with no confirmation anywhere on
  // the path. Deleting the whole vault deliberately is what the Quarantine
  // Manager's Delete Forever is for, and it is behind a typed double
  // confirmation.
  merged.autoPurgeRetentionDays = clampInt(merged.autoPurgeRetentionDays, 1, 3650, 30);
