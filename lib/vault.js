...
const { lockFailuresFrom, annotateLockFailures } = require('./lock-failure');
...
async function quarantine(items, options = {}) {
  ...
  // The renderer decides whether to offer "Find what is holding it" from this
  // flag now, instead of re-running the classifier on the error string. One
  // rule, one place, and the value travels with the row it describes.
  annotateLockFailures(entry);

  // z71u: the payload has ALREADY been moved into the vault by the time we get
  // here. If the manifest cannot be read, store.addManifestEntry now throws
  // rather than writing an empty one over it - which is the right call, and it
  // leaves this entry unrecorded. Say so, loudly, instead of letting the throw
  // become a generic quarantine failure: the difference matters, because the
  // files are safe and in the vault either way, and the user needs to know
  // where they are and that the OTHER entries were not touched.
  let indexed = true;
  let indexError = null;
  if (result.quarantinedCount > 0) {
    try {
      ...
      store.appendOplog({
        action: 'vault-manifest-unwritable',
        tier: 'full',
        items: { entryId, quarantined: result.quarantinedCount },
        outcome: 'error',
        meta: { error: err.message, code: err.code || null, vaultPath: path.join(store.vaultRoot(), entryId) }
      });
    }
  }
  ...
  // 2brn: capped at the number store.lockedPaths() will ever read back, with
  // the remainder counted rather than silently dropped. Anything past the
  // fiftieth was being written, never read, and still spending the oplog's
  // 5 MB rotation budget - which evicts the OLDEST records, so unreadable rows
  // were pushing out the elevation history the diagnostic collector needs.
  const locked = lockFailuresFrom(entry);
  const lockedPaths = locked.paths;
  ...
  store.appendOplog({
    ...
    items: {
      ...
      // Only when it happened, so an ordinary record does not carry a zero
      // that reads as a claim.
      ...(locked.dropped ? { lockedPathsDropped: locked.dropped } : {})
    },
  ...
  return {
    ...
    // z71u: only ever false when the manifest could not be read. The removal
    // itself succeeded and the payload is in the vault; what failed is the
    // record of it, which is the difference between "your files are gone" and
    // "your files are here and Vanish cannot list them yet".
    indexed,
    indexError
  };
}
...
async function autoPurgeSweep() {
  ...
  // z71u: this is the one unattended destructive path in the product - it runs
  // at every elevated start with no prompt. A manifest it cannot read is
  // exactly when it must do nothing: it would otherwise be deciding what to
  // delete forever from a list it could not obtain. Refusing is silent to the
  // user by design (so is a sweep that finds nothing), but it is recorded.
  let due;
  try {
    due = store
      .readManifest()
    ...
    return { swept: 0, skipped: 'manifest-unreadable', error: err.message };
  }
